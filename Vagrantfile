...
def setup_dns(node)
  ...
  node.vm.provision "setup-hosts", :type => "shell", :path => "ubuntu/vagrant/setup-hosts.sh" do |s|
    s.args = [IP_NW, BUILD_MODE, NUM_MASTER_NODES, NUM_WORKER_NODES, NUM_LBS, MASTER_IP_START, NODE_IP_START, LB_IP_START]
  end
  ...
  (1..NUM_MASTER_NODES).each do |i|
    config.vm.define "master-#{i}" do |node|
      ...
      if BUILD_MODE == "BRIDGE"
        node.vm.network :public_network, bridge: get_bridge_adapter(), ip: IP_NW + ".#{MASTER_IP_START + i}"
      else
      ...
      node.vm.provision "shell", inline: <<-SHELL
        ...
        # Configure static IP
        ip addr add #{IP_NW}.#{MASTER_IP_START + i}/24 dev $interface_name 
